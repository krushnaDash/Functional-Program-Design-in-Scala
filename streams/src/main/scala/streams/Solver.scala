...
trait Solver extends GameDef:
  ...
  def newNeighborsOnly(neighbors: LazyList[(Block, List[Move])],
                       explored: Set[Block]): LazyList[(Block, List[Move])] = neighbors.filterNot(explored contains _._1)
  ...
  def from(initial: LazyList[(Block, List[Move])],
    ...
    if (initial.isEmpty) LazyList.empty
      ...
      val newPaths = for {
        ...
        next <- newNeighborsOnly(neighborsWithHistory(block, moveList), explored)
      } yield next
      initial #::: from(newPaths, explored ++ (newPaths map (_._1)))
    }
  }
