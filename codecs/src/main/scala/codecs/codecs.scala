...
trait DecoderInstances:
  ...
  given Decoder[Int] =
    Decoder.fromPartialFunction { case Json.Num(n) => n.intValue }

  /** A decoder for `String` values */
  // TODO Define a given value of type `Decoder[String]`

  given Decoder[String] =
    Decoder.fromPartialFunction { case Json.Str(n) => n.toString }

  /** A decoder for `Boolean` values */
  // TODO Define a given value of type `Decoder[Boolean]`

  given Decoder[Boolean] =
    Decoder.fromPartialFunction { case Json.Str(n) => n.toBoolean }
...
object Main:
  ...
  def main(args: Array[String]): Unit =
    ...
    println(renderJson(List(Person("Krushna", 35),Person("Shweta", 35))))
